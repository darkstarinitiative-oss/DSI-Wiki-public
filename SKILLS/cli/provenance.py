...
brief/changelog/devlog) plus archive and fact-check backup dirs under the instance's
...
CODE/gateway/api_app.py, scan_dir from its HTTP config. No hardcoded dirs.
...
_SKILLS_DIR = os.path.dirname(_CLI_DIR)
_REPO_ROOT = os.path.dirname(_SKILLS_DIR)
_HTTP_DIR = os.path.join(_REPO_ROOT, "CODE", "gateway")
sys.path.insert(0, _HTTP_DIR)

import api_app  # noqa: E402

_HTTP_CONFIG = os.path.join(_REPO_ROOT, "JSONS", "DSI-Wiki-HTTP-Config.json")
...
def _load_scan_dir():
    try:
        ...
        pass
    return os.path.join(_REPO_ROOT, "JSONS", "instances")
